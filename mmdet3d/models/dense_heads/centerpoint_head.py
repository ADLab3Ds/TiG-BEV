...
@HEADS.register_module()
class CenterHeadKD(CenterHead):
    @force_fp32(apply_to=('preds_dicts'))
    def loss(self, gt_bboxes_3d, gt_labels_3d, preds_dicts, **kwargs):
        heatmaps_kd = []
        #heatmaps--list heatmaps[0] B,class_num,128,128
        heatmaps, anno_boxes, inds, masks = self.get_targets(
            gt_bboxes_3d, gt_labels_3d)
        loss_dict = dict()
        for task_id, preds_dict in enumerate(preds_dicts):
            ...
            heatmaps_kd.append((preds_dict[0]['heatmap']*heatmaps[task_id]).detach())
            num_pos = heatmaps[task_id].eq(1).float().sum().item()
            loss_heatmap = self.loss_cls(
                preds_dict[0]['heatmap'],
                heatmaps[task_id],
            ...
            if self.task_specific:
                ...
                loss_dict[f'task{task_id}.loss_bbox'] = loss_bbox
            loss_dict[f'%stask{task_id}.loss_heatmap'%(self.loss_prefix)] = loss_heatmap
        heatmaps_kd = torch.cat(heatmaps_kd,dim=1)
        return loss_dict,heatmaps_kd