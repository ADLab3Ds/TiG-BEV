...
def merge_aug_bboxes_3d(aug_results, img_metas, test_cfg):
    ...
    for class_id in range(torch.max(aug_labels).item() + 1):
        ...
        if len(bboxes_nms_i) == 0:
            continue
        selected = nms_gpu(bboxes_nms_i, scores_i, test_cfg.nms_thr[test_cfg.ind2class[class_id]])
        #selected = nms_gpu(bboxes_nms_i, scores_i, test_cfg.nms_thr[0])
        '''if test_cfg.nms_type[test_cfg.ind2class[class_id]] == 'rotate':
            selected = nms_gpu(bboxes_nms_i, scores_i, test_cfg.nms_thr[test_cfg.ind2class[class_id]])
        else:
            selected = nms_normal_gpu(bboxes_nms_i, scores_i, test_cfg.nms_thr[test_cfg.ind2class[class_id]])'''
    ...
    num = min(test_cfg.max_per_img, len(aug_bboxes))
    #num = min(test_cfg.post_max_size, len(aug_bboxes))
