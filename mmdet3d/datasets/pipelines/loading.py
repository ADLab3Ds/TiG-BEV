...
class PointToMultiViewDepthWithGTIndex(object):
    """Use points to calculate the depth value and the foreground target index of multi-view images.

    Args:
        grid_config (dict): Configuration dict of the bev grid.
        downsample (int): The downsample rate of the feature map.
    """
    ...
    def points2map(self,points_label,points, height, width):
        ...
        Args:
            points_label (torch.Tensor): A binary tensor, '1' means the foreground point and '0' means the background point.
            points (torch.Tensor): Coordinate values of points.
            height (torch.float): The height value of the image before downsampling.
            width (torch.float): The width value of the image before downsampling.

        Returns:
            torch.Tensor: Depth value of each pixel.
            torch.Tensor: The index of box where each pixel are in. '0' indicates the background pixel.
        ...
        points_label, coor, depth = points_label[:,kept1], coor[kept1], depth[kept1]
        ranks = coor[:, 0] + coor[:, 1] * width
        sort = (ranks+depth/100.).argsort()
        points_label, coor, depth, ranks = points_label[:,sort], coor[sort], depth[sort], ranks[sort]

        kept2 = torch.ones(coor.shape[0], device=coor.device, dtype=torch.bool)
        kept2[1:] = (ranks[1:] != ranks[:-1])
        points_label, coor, depth = points_label[:,kept2], coor[kept2], depth[kept2]
        coor = coor.to(torch.long)
        depth_map[coor[:,1],coor[:,0]] = depth
        fg_bbox_ind, fg_points_ind = torch.where(points_label == 1) 
        fg_coor = coor[fg_points_ind] 
        fg_map = torch.zeros((height,width), dtype=torch.long)
        fg_map[fg_coor[:,1],fg_coor[:,0]] = fg_bbox_ind + 1 
        return depth_map, fg_map
        

    def __call__(self, results):
        """Call function to use points to calculate the depth value and the foreground target index of multi-view images.

        Args:
            results (dict): Result dict containing multi-view image data.

        Returns:
            dict: The result dict containing the multi-view image data. The value of the key 'img_inputs' will be updated.
                  Added values of results['img_inputs'] are described below.

                - depth_map_multiview (torch.Tensor): Foreground target indices of multi-view image arrays.
                - fg_map_multiview (torch.Tensor): Depth values of multi-view image arrays.
        """
        if 'cur_points_num' in results.keys():
            points_lidar = results['points'][:results['cur_points_num']]
        else:
            points_lidar = results['points']
        points_label = results['gt_bboxes_3d'].points_in_boxes(points_lidar.coord) #bbox_num,points_num
        ...
        for cid in range(rots.shape[0]):
            ...
            depth_map, fg_map = self.points2map(points_label,points_img, imgs.shape[2], imgs.shape[3])
            
            depth_map_list.append(depth_map)
            fg_map_list.append(fg_map)
            
        depth_map_multiview = torch.stack(depth_map_list)
        fg_map_multiview = torch.stack(fg_map_list)
        
        results['img_inputs'] = (imgs, rots, trans, intrins, post_rots, post_trans, 
                                 depth_map_multiview,fg_map_multiview) 
        return results
...
class LoadMultiViewImageFromFiles_BEVDet(object):
    ...
    def __init__(self, data_config, is_train=False,
                 sequential=False, aligned=False, trans_only=True,get_img_before_norm=False):
        ...
        self.img2tensor = torchvision.transforms.Compose([
            torchvision.transforms.ToTensor()])
        self.sequential = sequential
        self.aligned = aligned
        self.trans_only = trans_only
        self.get_img_before_norm = get_img_before_norm
    ...
    def get_inputs(self,results, flip=None, scale=None):
        ...
        for cam in cams:
            ...
            if self.get_img_before_norm:
                imgs_before_norm.append(self.img2tensor(img))
            ...
            if self.sequential:
                ...
                if not type(results['adjacent']) is list:
                    ...
                    img_adjacent = self.img_transform_core(img_adjacent,
                                                           ...
                                                           rotate=rotate)
                    if self.get_img_before_norm:
                        imgs_before_norm.append(self.img2tensor(img_adjacent))
        ...
        if self.sequential:
            if self.trans_only:
                ...
                if not type(results['adjacent']) is list:
                    ...
                    assert False
        if self.get_img_before_norm:
            imgs_before_norm,imgs, rots, trans, intrins, post_rots, post_trans = (torch.stack(imgs_before_norm),torch.stack(imgs), torch.stack(rots), torch.stack(trans),
        ...
        results['img_inputs'] = (imgs, rots, trans, intrins, post_rots, post_trans)
        if self.get_img_before_norm:
            results['imgs_before_norm'] = imgs_before_norm
       
        return results

    def __call__(self, results):
        results = self.get_inputs(results)
        return results
...
class LoadPointsFromMultiSweeps(object):
    ...
    def __call__(self, results):
        ...
        cur_points_num = points.shape[0] # num of current points
        points = points.cat(sweep_points_list)
        points = points[:, self.use_dim]
        results['points'] = points
        results['cur_points_num'] = cur_points_num
        return results
